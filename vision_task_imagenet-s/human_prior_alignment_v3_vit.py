...
# 训练
# -------------------
def train_one_epoch(model, optimizer, scaler, loader, device, args, epoch, human_lima):
    model.train()
    ...
    for images, masks, labels, img_paths, mask_paths in pbar:
        ...
        if aug_step_count % (args.align_steps + 1) == 0:
            ...
            for selected_img_path, selected_image, selected_mask, selected_label in zip(
                ...
                image_tmp = cv2.resize(cv2.imread(selected_img_path), (224,224))  # BGR; 若算法依赖颜色，必要时转 RGB
            ...
            if len(samples_after_none_human) != 0:  
                ...
                # 计算logits
                with torch.autocast("cuda", enabled=args.amp):
                    inserts_after = model(samples_after_none_human).softmax(dim=-1)  # [B, D]
                    dels_after = model(samples_after_none_human_reverse).softmax(dim=-1)  # [B, D]
                    
                with torch.no_grad():
                    inserts_before = model(samples_before_none_human).softmax(dim=-1)  # [B, D]
                    dels_before = model(samples_before_none_human_reverse).softmax(dim=-1)  # [B, D]
                ...
                margin = 0.1  # 软间隔阈值，可调
                loss_redundancy = (
                    F.relu(gt_scores_inserts_after - gt_scores_inserts_before.detach() - margin).mean() +
                    F.relu(gt_scores_dels_before.detach() - gt_scores_dels_after - margin).mean()
                )
...
def main_worker():
    ...
    parser.add_argument("--output_dir", default="./ckpt_vision_imagenets/ckpts_vit_b16_imnet_human_prior_v3_detach/")
    ...
    human_lima = HumanLIMA(model, threshold=args.threshold, softmax=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_one_epoch(model, optimizer, scaler, train_loader, device, args, epoch, human_lima)
