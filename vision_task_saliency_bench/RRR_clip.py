...
def train_one_epoch(model, optimizer, scaler, loader, device, tokenizer, idx_to_label, args, epoch, text_feats):
    ...
    for images, masks, labels, img_paths, mask_paths in pbar:
        ...
        # 求输入梯度：d(target_score)/d(images)
        # create_graph=True 才能让梯度惩罚项继续对参数反传（RRR 必需）
        ...
        # mask broadcast 到通道维
        if masks.dim() == 3:
            masks_ = masks.unsqueeze(1)           # [B,1,H,W]
        else:
            masks_ = masks                         # [B,1,H,W] expected

        # RRR 风格：惩罚 mask=1 区域的梯度平方
        loss_r = ((1 - masks_) * grads.pow(2)).mean()
        
        # 总损失
        loss_all = loss_ce + 0.1 * loss_r
...
def main_worker():
    ...
    parser.add_argument("--output_dir", default="./ckpt_vision_saliency_bench/ckpts_clip_L14_RRR")
