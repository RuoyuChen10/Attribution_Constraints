    ...
    for images, masks, labels, img_paths, mask_paths in pbar:
        ...
        if aug_step_count % (args.align_steps + 1) == 0:
            ...
            if len(samples_first_suppress) != 0:
                ...
                scaler.scale(0.5 *loss_human).backward()
...
#             loss = ce(logits, labels)  # labels: [B], 取值范围 [0, C-1]
...
#                 # 取置信度大于threshold的，一般设置0.75
#                 scores = F.softmax(logits, dim=-1)
#                 preds = scores.argmax(dim=-1)        # [B]
#                 confidences = scores[torch.arange(len(scores)), preds]  # [B]
                
#                 mask = (preds == labels) & (confidences > args.threshold)
                
#                 # 取出符合条件的索引
#                 selected_indices = torch.nonzero(mask, as_tuple=True)[0]
                
#                 # 1️⃣ 转成 Python list 索引
#                 selected_indices_list = selected_indices.cpu().tolist()
#                 # 2️⃣ 用列表推导式筛选
#                 selected_img_paths = [img_paths[i] for i in selected_indices_list]
#                 selected_masks = masks[selected_indices.cpu()]
...
#                 for selected_img_path, selected_image, selected_mask, selected_label in zip(selected_img_paths, selected_images, selected_masks, selected_labels):
#                     # 读取图像
#                     image_tmp = cv2.imread(selected_img_path)
...
#                     elif human_consistency_judge[0] == False:
#                         # 如果第一个都不符合，就不进行任何遮挡
#                         samples_first_suppress.append(selected_image * S_tensor[0].unsqueeze(0))
#                         samples_first_suppress_reverse.append(selected_image * (~S_tensor[0].unsqueeze(0)))
#                         samples_first_suppress_labels.append(selected_label)
#                     else:
#                         # 第一个对了，但是后面的不好
#                         false_indices = torch.nonzero(~human_consistency_judge[1:], as_tuple=True)[0] + 1
#                         for false_indice in false_indices:
#                             samples_after_none_human.append(
#                                 S_tensor[:false_indice+1].sum(0).clamp(max=1).unsqueeze(0) * selected_image
#                             )
#                             samples_after_none_human_reverse.append(    
#                                 (1 - S_tensor[:false_indice+1].sum(0).clamp(max=1)).unsqueeze(0) * selected_image
#                             )
#                             samples_before_none_human.append(
#                                 S_tensor[:false_indice].sum(0).clamp(max=1).unsqueeze(0) * selected_image
#                             )
#                             samples_before_none_human_reverse.append(    
#                                 (1 - S_tensor[:false_indice].sum(0).clamp(max=1)).unsqueeze(0) * selected_image
#                             )
#                             samples_none_human_labels.append(selected_label)
#                     # 结束循环
#                 model.train()
...
#                 if len(samples_first_suppress) != 0:
#                     samples_first_suppress_labels = torch.stack(samples_first_suppress_labels)
                    
#                     samples_first_suppress = torch.stack(samples_first_suppress, dim=0)
#                     samples_first_suppress_reverse = torch.stack(samples_first_suppress_reverse, dim=0)
                    
#                     # 计算logits
#                     img_feats_suppress = model.get_image_features(pixel_values=samples_first_suppress)  # [B, D]
#                     img_feats_suppress = img_feats_suppress / img_feats_suppress.norm(dim=-1, keepdim=True)
                    
#                     img_feats_suppress_reverse = model.get_image_features(pixel_values=samples_first_suppress_reverse)  # [B, D]
#                     img_feats_suppress_reverse = img_feats_suppress_reverse / img_feats_suppress_reverse.norm(dim=-1, keepdim=True)
                    
#                     inserts = F.softmax((img_feats_suppress @ text_feats) * model.logit_scale.exp(), dim=-1)
#                     dels = F.softmax((img_feats_suppress_reverse @ text_feats) * model.logit_scale.exp(), dim=-1)
                    
#                     B = inserts.size(0)
#                     labels = samples_first_suppress_labels.to(device=inserts.device, dtype=torch.long)
                    
#                     gt_scores_inserts = inserts[torch.arange(B, device=inserts.device), labels]
#                     gt_scores_dels = dels[torch.arange(B, device=dels.device), labels]
                    
#                     loss_human = (gt_scores_inserts).mean() + (1 - gt_scores_dels).mean()   # 压制inserts，提升dels
#                     print("Loss_human:", loss_human.item())
                    
#                     scaler.scale(loss_human).backward()
#                     scaler.step(optimizer)
...
#                 if len(samples_after_none_human) != 0:
#                     # 预先 stack 一次再分批，避免每个 batch 都 stack
#                     samples_after_none_human       = torch.stack(samples_after_none_human, dim=0)
#                     samples_before_none_human      = torch.stack(samples_before_none_human, dim=0)
#                     samples_after_none_human_rev   = torch.stack(samples_after_none_human_reverse, dim=0)
#                     samples_before_none_human_rev  = torch.stack(samples_before_none_human_reverse, dim=0)
#                     labels_all = torch.stack(samples_none_human_labels).to(device, dtype=torch.long)
                    
#                     batch_size = 8          # 按显存调
#                     accum_steps = 2         # 梯度累积步数（等效更大batch）
#                     num_samples = samples_after_none_human.size(0)
...
#                         after_batch     = samples_after_none_human[s:e].to(device, non_blocking=True)
#                         before_batch    = samples_before_none_human[s:e].to(device, non_blocking=True)
#                         after_rev_batch = samples_after_none_human_rev[s:e].to(device, non_blocking=True)
#                         before_rev_batch= samples_before_none_human_rev[s:e].to(device, non_blocking=True)
#                         labels_batch    = labels_all[s:e]

#                         # with torch.autocast("cuda", enabled=True):  # 或 enabled=args.amp
#                         # ---- 获取图像特征并单位化 ----
#                         img_after = model.get_image_features(pixel_values=after_batch)
#                         img_after = img_after / img_after.norm(dim=-1, keepdim=True)

#                         img_before = model.get_image_features(pixel_values=before_batch)
#                         img_before = img_before / img_before.norm(dim=-1, keepdim=True)

#                         img_after_rev = model.get_image_features(pixel_values=after_rev_batch)
#                         img_after_rev = img_after_rev / img_after_rev.norm(dim=-1, keepdim=True)

#                         img_before_rev = model.get_image_features(pixel_values=before_rev_batch)
#                         img_before_rev = img_before_rev / img_before_rev.norm(dim=-1, keepdim=True)

#                         # ---- 计算 logits -> 概率（注意 dim=-1）----
#                         # (B, D) @ (D, C) = (B, C)
#                         scale = model.logit_scale.exp()
#                         inserts_after  = (img_after     @ text_feats) * scale
#                         inserts_before = (img_before    @ text_feats) * scale
#                         dels_after     = (img_after_rev @ text_feats) * scale
#                         dels_before    = (img_before_rev@ text_feats) * scale

#                         inserts_after  = inserts_after.softmax(dim=-1)
#                         inserts_before = inserts_before.softmax(dim=-1)
#                         dels_after     = dels_after.softmax(dim=-1)
#                         dels_before    = dels_before.softmax(dim=-1)

#                         B = inserts_after.size(0)
#                         ar = torch.arange(B, device=labels_batch.device)

#                         gt_ins_after  = inserts_after[ar, labels_batch]
#                         gt_ins_before = inserts_before[ar, labels_batch]
#                         gt_del_after  = dels_after[ar, labels_batch]
#                         gt_del_before = dels_before[ar, labels_batch]

#                         # 你的目标：压制 inserts（after-before>0 罚），提升 dels（before-after>0 罚）
#                         loss_batch = F.relu(gt_ins_after - gt_ins_before).mean() + \
#                                     F.relu(gt_del_before - gt_del_after).mean()

#                         # 若做梯度累积，这里要除以 accum_steps，等效平均
#                         loss_batch_scaled = loss_batch / accum_steps

#                         # ---- AMP 回传 ----
#                         scaler.scale(loss_batch_scaled).backward()
#                         loss_meter += loss_batch.item()

#                         # ---- 每 accum_steps 步做一次优化器更新 ----
#                         if ((bi + 1) % accum_steps == 0) or (bi == num_batches - 1):
#                             scaler.step(optimizer)
#                             scaler.update()
#                             optimizer.zero_grad(set_to_none=True)
                    
#         aug_step_count += 1
                    # samples_none_human_labels = torch.stack(samples_none_human_labels)
                    
                    # samples_after_none_human = torch.stack(samples_after_none_human, dim=0)
                    # samples_before_none_human = torch.stack(samples_before_none_human, dim=0)
                    
                    # samples_after_none_human_reverse = torch.stack(samples_after_none_human_reverse, dim=0)
                    # samples_before_none_human_reverse = torch.stack(samples_before_none_human_reverse, dim=0)
                    
                    # # 计算logits
                    # img_feats_after = model.get_image_features(pixel_values=samples_after_none_human)  # [B, D]
                    # img_feats_after = img_feats_after / img_feats_after.norm(dim=-1, keepdim=True)
                    
                    # img_feats_before = model.get_image_features(pixel_values=samples_before_none_human)  # [B, D]
                    # img_feats_before = img_feats_before / img_feats_before.norm(dim=-1, keepdim=True)
                    
                    # img_feats_after_reverse = model.get_image_features(pixel_values=samples_after_none_human_reverse)  # [B, D]
                    # img_feats_after_reverse = img_feats_after_reverse / img_feats_after_reverse.norm(dim=-1, keepdim=True)
                    
                    # img_feats_before_reverse = model.get_image_features(pixel_values=samples_before_none_human_reverse)  # [B, D]
                    # img_feats_before_reverse = img_feats_before_reverse / img_feats_before_reverse.norm(dim=-1, keepdim=True)
                    
                    # inserts_after = F.softmax((img_feats_after @ text_feats) * model.logit_scale.exp(), dim=-1)
                    # inserts_before = F.softmax((img_feats_before @ text_feats) * model.logit_scale.exp(), dim=-1)
                    
                    # dels_after = F.softmax((img_feats_after_reverse @ text_feats) * model.logit_scale.exp(), dim=-1)
                    # dels_before = F.softmax((img_feats_before_reverse @ text_feats) * model.logit_scale.exp(), dim=-1)
                    
                    # B = inserts_after.size(0)
                    # labels = samples_none_human_labels.to(device=inserts_after.device, dtype=torch.long)
                    
                    # gt_scores_inserts_after = inserts_after[torch.arange(B, device=inserts_after.device), labels]
                    # gt_scores_inserts_before = inserts_before[torch.arange(B, device=inserts_before.device), labels]
                    
                    # gt_scores_dels_after = dels_after[torch.arange(B, device=dels_after.device), labels]
                    # gt_scores_dels_before = dels_before[torch.arange(B, device=dels_before.device), labels]
                    
                    # loss_redundancy = F.relu(gt_scores_inserts_after - gt_scores_inserts_before).mean() + F.relu(gt_scores_dels_before - gt_scores_dels_after).mean()   # 压制inserts，提升dels
...
def main_worker():
    ...
    parser.add_argument("--output_dir", default="./ckpts_clip_L14_human_prior")
